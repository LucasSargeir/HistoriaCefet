...
<html>
	<head>
		<title>Texto dos Estagiários</title>
		<meta charset="utf-8" />
							...
							<!-- Content -->
								<section>
									<header class="main">
										...
										<form method="post" action="textoTE.php">
										...
										<?php
											if(isset($_REQUEST['materia']) || isset($_REQUEST['titulo'])){
												...
												switch ($aux){
													case 1:
														$sql = "select * from textoaluno where idM = $mat";													
														break;
													
													case 2:
														$sql = "select * from textoaluno where nome like '%$tit%'";													
														break;
													
													case 3:
														$sql = "select * from textoaluno where idM='$mat' and nome like '%$tit%'";													
														break;
												...
												//echo"$sql";
												$resposta = mysqli_query($link, $sql." and aluno = 0 order by id_textoA desc");
												...
												if($resposta){
													...
													while($linhaTextos){
														...
														echo "<article >
															...
															<b>ARstagiário(s):</b> {$linhaTextos['nomeA']} <br><br>
																    {$linhaTextos['resumo']} 
															</div>
																<br>

																<ul class='actions'>
																	<li><a href='texto.php?id={$linhaTextos['id_textoA']}&aluno=0' class='button special'>Mais</a></li>
																</ul></article>";
												...
												$linhaTextos = recebeTextosA($pagT,16,"textoaluno","id_textoA","aluno = 0 and", $link);

												echo"<header class='major'>
														<h2>Textos dos Estagiários</h2>
													</header><div class='posts' >";
												...
												for ($i=0; $i < count($linhaTextos); $i++) { 
													...
													echo"</h6></div>
													<div style='overflow: hidden; width:25em; border:1px; text-overflow: ellipsis;  white-space:nowrap;'>
															<b>Estagiário(s):</b> {$linhaTextos[$i]['nomeA']} <br><br>
														    {$linhaTextos[$i]['resumo']} 
													</div>
														<br>

														<ul class='actions'>
															<li><a href='texto.php?id={$linhaTextos[$i]['id_textoA']}&aluno=0' class='button special'>Mais</a></li>
														</ul></article>";
												}
												}
												...
												$totalObj = mysqli_num_rows(mysqli_query($link, "select * from textoaluno"));
												...
												if($pagT == 1 ){
													...
													$sub = $pagT - 1;
													echo" <li><a href='textoTE.php?pagT=$sub' class='button'>Voltar</a></li>";
												...
												for ($i=1; $i <= $numPag ; $i++) { 
													if( $i == $pagT){
														...
														echo"<li><a href='textoTE.php?pagT=$i' class='page active'>$i</a></li>";

													}else{

														echo"<li><a href='textoTE.php?pagT=$i' class='page'>$i</a></li>";
												...
												if($pagT == $numPag ){
													...
													$som = $pagT + 1;
													echo" <li><a href='textoTE.php?pagT=$som' class='button'>Próximo</a></li>";
