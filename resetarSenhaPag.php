								...
								<section>
									...
									<form name="form" method="post" action="cTexto.php" enctype="multipart/form-data">
									    <fieldset class="infraFieldset">
									        <h2 class="infraLegend">Todas os Professores/h2>
									        <h3>Clique nos nomes para resetar a senha, senha padrão ao resetar: 12345678</h3>
											<?php
												$resposta3 = mysqli_query($link, "select * from professor where id_professor='$idA'");
												if($resposta3){	
													$linha3 = mysqli_fetch_assoc($resposta3);
													if($linha3['admin']){
														$resposta2 = mysqli_query($link, "select * from professor");
														if($resposta2){	

															$linha2 = mysqli_fetch_assoc($resposta2);
															while($linha2){		
																echo"<a href='resetarSenha.php?id={$linha2['id_professor']}&email={$linha2['email']}'>{$linha2['nome']}</a>&nbsp;&nbsp;&nbsp;&nbsp;";
																$linha2 = mysqli_fetch_assoc($resposta2);
															}
														}
													}
